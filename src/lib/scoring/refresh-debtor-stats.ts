...
/**
 * Recalcule risk_score, risk_category, avg_payment_delay, late_invoice_rate,
 * ca_percentage et total_outstanding pour un débiteur donné.
 *
 * Source des stats :
 *   - avg_payment_delay : moyenne (date_paiement - due_date) sur les
 *     factures payées des 12 derniers mois. Pas de date de paiement directe
 *     dans le schéma → on utilise updated_at de la facture quand status=paid.
...
export const refreshAllDebtorStats = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    for (const d of debtors ?? []) {
      try {
        await refreshDebtorStats({ data: { debtorId: d.id } });
        processed++;
      } catch {
        errors++;
      }
